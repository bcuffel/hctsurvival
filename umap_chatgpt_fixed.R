...
set.seed(123)
splits <- initial_split(surv_data, strata = risk_group)
...
# -------------------------
# 9. Predict Survival Probabilities
# -------------------------
...
new_survival_probs <- predict(last_surv_fit, test_data %>% select(-surv_obj, -risk_group), 
                              type = "survival", eval_time = time_grid)
